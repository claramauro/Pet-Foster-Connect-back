...
async function sendConfirmationEmailProd(email) {
  ...
  const msg = {
    ...
    </html>`,  // Optionnel, pour email HTML
  };
  ...
  sgMail
    .send(msg)
    .then(() => {
      console.log('Email envoyé avec succès'); // Succès de l'envoi du mail 
    })
    .catch((error) => {
      console.error('Erreur d\'envoi:', error); // Erreur de l'envoi du mail
    });
}
